            ...
            <CardContent className="pt-6">
              ...
              {(data.userExits.length === 0 || !data.assemblyPoint) && (
                <p className="text-sm text-amber-600 mt-2">
                  ?? Required: {data.userExits.length === 0 && 'At least 1 exit'}
        ...
        <Card>
          <CardContent className="py-12 text-center">
            ...
            <div className="text-sm text-muted-foreground space-y-1">
              <p>?? Elapsed: {pollingInfo.elapsedSeconds}s</p>
              <p>?? Status: {pollingInfo.lastStatus}</p>
              {pollingInfo.elapsedSeconds > 20 && pollingInfo.elapsedSeconds <= 60 && (
                <p className="text-amber-600 mt-2">
                  ?? Simulation is computing fire spread and agent movement...
                </p>
              )}
              {pollingInfo.elapsedSeconds > 60 && pollingInfo.elapsedSeconds <= 180 && (
                <p className="text-amber-600 mt-2">
                  ?? Large simulations may take 1-3 minutes. Please wait...
                </p>
              )}
              {pollingInfo.elapsedSeconds > 180 && (
                <p className="text-amber-600 mt-2">
                  ?? Simulation is taking longer than usual. It will timeout at 5 minutes.
                </p>
              )}
            </div>
            {data.jobId && (
